...
const STORE_KEY_SELECTED = 'selected', STORE_KEY_MOD = 'selectedMod', STORE_KEY_DEBOUNCE = 'selectedDebounce',
	STORE_KEY_POSTER = 'selectedPoster', STORE_KEY_LANG = 'lang', STORE_KEY_RATE = 'selectedRate',
	STORE_KEY_SERVER = 'server', STORE_KEY_VERSION = 'dataVersion';
...
var dataConvert20200120 = function (data, data2) {
	var num = ["ex007", "ex019", "ex004", "ex005", "ex020", "ex021", "ex002", "ex008", "ex022", "ex001",
		"ex003", "ex006", "ex009", "ex010", "ex017", "ex018"];
	var num2 = ["1007", "1019", "1004", "1005", "1020", "1021", "1002", "1008", "1022", "1001", "1003",
		"1006", "1009", "1010", "1017", "1018"];

	for (var n in num) {
		var idx = data.guns.indexOf(num[n]);
		if (idx > -1) {
			data.guns.splice(idx, 1);
			data.guns.push(num2[n])
		}
	}
	var list = [];
	for (var n in data.guns) {
		list.push("" + parseInt(data.guns[n]))
	}
	data.guns = list;

	var list2 = [];
	for (var n in data2.guns) {
		list2.push(parseInt(data2.guns[n]) + "m")
	}
	data2.guns = list2;
		...
		if (gun[server + 'exist']) {
			...
			if (gun.mods && (server == 'cn' || gun.mods.mod3[server + 'exist'])) {
				var itemMod = {
					...
					"twname": gun.mods.mod3.twname,
					"jpname": gun.mods.mod3.jpname,
					"krname": gun.mods.mod3.krname,
					"no": gun.mods.mod3.no,
	...
	var app = new Vue({
		...
		methods: {
			...
			storeAll: function () {
				...
				this.store(STORE_KEY_VERSION, dataVersion)
				this.msg = this.$i18n.messages[this.lang].message.saveCfgDone;
				this.toast = this.$i18n.messages[this.lang].message.saveCfgDone;
				this.showToast = true;
			...
			restoreAll: function () {
				...
				let s = this.restore(STORE_KEY_SELECTED), smod = this.restore(STORE_KEY_MOD), sposter = this.restore(STORE_KEY_POSTER),
					...
					server = this.restore(STORE_KEY_SERVER), version = this.restore(STORE_KEY_VERSION);
				if (s == undefined) {
					this.msg = this.$i18n.messages[this.lang].message.loadCfgEmpty;
					this.toast = this.$i18n.messages[this.lang].message.loadCfgEmpty;
					this.$bvToast.toast(`${this.toast}`, {
						title: 'Message',
						solid: true,
						variant: 'warning',
					...
					if (version == undefined) {
						result = dataConvert20200120(sposter, smod);
						sposter = result.data;
						smod = result.data2;
					}
					...
					this.$refs.mod_55m[0].select(this.selectedMod.guns);
					this.restored = false;
			...
			exportJSON: function () {
				...
				data[STORE_KEY_VERSION] = dataVersion;
				...
				if (input.files && input.files[0]) {
					...
					reader.onload = (e) => {
						...
						let s = data[STORE_KEY_SELECTED], smod = data[STORE_KEY_MOD], sposter = data[STORE_KEY_POSTER],
							...
							server = data[STORE_KEY_SERVER], version = data[STORE_KEY_VERSION];
						if (version == undefined) {
